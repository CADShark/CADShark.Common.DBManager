...
Точный маршрут и DTO загрузки файла ДОЛЖНЫ быть подтверждены по существующему OpenVault API до реализации вызова.
...
| `API-ADD-01` | OpenVault API | Подтвердить маршрут и DTO загрузки основного файла. |
| `CLIENT-ADD-01` | OpenManage.Client | Добавить только типизированный вызов серверной загрузки. |
...
### OQ-003. Контракт загрузки файла

Необходимо подтвердить фактический маршрут OpenVault API и DTO, через которые основной файл загружается для конкретного Version Object ID.
...
| `0.4` | 2026-08-13 | Управление файлами закреплено за OpenVault API; Storage исключён из wrapper; текущий этап переведён в режим CreateOnly. |
